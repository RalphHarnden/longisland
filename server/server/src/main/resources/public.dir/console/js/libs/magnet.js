...
magnet.global = {
    ...
    host      : 'localhost',
    port      : 8080,
    baseUrl   : window.location.href.replace(window.location.hash, '').substr(0, window.location.href.replace(window.location.hash, '').lastIndexOf('/')).replace('/console', '')
}
magnet.tabs = {
...
magnet.dataMgr = {
    ...
    typeDomId : 'reqType',
    initPoll  : true,
    call : function(loc, method, dataType, contentType, data, callback, doLogging, failFn, isPolling, pollDelay){
        var me = this;
        var dataStr = null;
        ...
        $.ajax({  
            ...
            error : function(xhr, ajaxOptions, thrownError){
                ...
                if(doLogging){
                    me.logHTTP(data, xhr, thrownError);
                }
                if(failFn){
    ...
    updateTables : function(container, sel, data){
        if(container){
            sel = container+' .'+sel;
        }
        $('#'+sel).html('');
        if(data !== undefined && data !== null){
            if(typeof data === 'string'){
                $('#'+sel).html('<pre>'+data+'</pre>');
            }else if(typeof data === 'object'){
                $('#'+sel).html('<table id="table_'+sel+'"><thead><tr><th>Name</th><th>Value</th></tr></thead><tbody></tbody></table>');
                $.each(data, function(id, val){
                    if(typeof val === 'object' || typeof val === 'array'){
                        var uniqueId = magnet.localStore.getID();
                        $('#'+sel+' tbody').append('<tr><td>'+id+'</td><td><div id="'+'tree'+uniqueId+'">'+magnet.tree.toTreeObj(val)+'</div></td></tr>');
                        magnet.tree.create('tree'+uniqueId);
                    }else{
                        $('#'+sel+' tbody').append('<tr><td>'+id+'</td><td>'+val+'</td></tr>');
                    }
                });
            }
        }
    ...
    loadTableStyles : function(){
        ...
        $.each(tblAry, function(i, val){
            $('#'+val+' tr:even, .'+val+' tr:even').removeClass().addClass('tr_even');
            $('#'+val+' tr:odd, .'+val+' tr:odd').removeClass().addClass('tr_odd');
        });
    },
    logHTTP : function(data, xhr, result, container){
        var me = this;
        if(data !== undefined){
            me.updateTables(container, 'request_data', data);
            me.updateTables(container, 'request_raw_data', magnet.utils.highLightJSON(data));
        }
        if(xhr !== undefined){
            ...
            me.updateTables(container, 'request_header', reqObj);
            me.updateTables(container, 'request_raw_header', magnet.utils.highLightJSON(reqObj));
            me.updateTables(container, 'response_header', resObj);
            me.updateTables(container, 'response_raw_header', magnet.utils.highLightJSON(resObj));
        }
        if(result !== undefined){
            me.updateTables(container, 'response_data', result);
            me.updateTables(container, 'response_raw_data', magnet.utils.highLightJSON(result));
        }
        me.loadTableStyles();
    }
}
...
magnet.tree = {
    ...
    init : function(clickback, callback, showChild){
        var me = this;
        magnet.dataMgr.call('/rest/entities.json', 'GET', 'json', 'application/json', {}, function(result){
            $('#data').removeClass('hidden');
            magnet.tree.create('apiTree', me.formatJSON(result, showChild), function(selNode, parNode){
                if(typeof clickback === typeof Function){
                    if(parNode != -1){
    ...
    formatJSON : function(data, showChild){
        ...
        $.each(data, function(i, obj){
            ...
            if(typeof showChild == 'undefined'){
                if(typeof entityObj.children === 'undefined'){
                    entityObj.children = [];
                }
    ...
    create : function(id, nodes, clickback, callback){
        ...
        if(nodes){
            tree.jstree({
                ...
                if(data.rslt.obj.hasClass('jstree-leaf') && typeof clickback === typeof Function){
            ...
            var urlRxp = /(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;
            tree.jstree({
                "core"    : {"animation"    : 200},
                "ui"      : {"select_limit" : 1},
                "themes"  : {"icons"        : false},
                ...
                if(urlRxp.test(data.rslt.obj.find('span').text())){
                    window.open(data.rslt.obj.find('span').text(), '_blank');
                }
            });
        }
    },
...
magnet.schema = {
    ...
    load : function(type, cb){
        ...
        if(me.cache[type] === undefined){
            magnet.dataMgr.call('/rest/schemas/'+type, 'GET', 'xml', '', null, function(xml){
                me.cache[type] = me.parse(type, xml);
                cb(me.cache[type].slice(0));
            }, false, function(){
                magnet.messages.create({
                    "title"   : "Connection Failure - xsd retrieval - "+type,
                    "message" : "The Developer Suite has no access to the APIs and cannot continue. Please check your connection and refresh the page again.",
                    "buttons" : [{
                        "title"  : "OK", 
                        "action" : function(){ 
